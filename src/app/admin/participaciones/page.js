...
export default function ParticipacionesPage() {
  ...
  const abrirEdicion = (part) => {
    ...
    setFormData({
      id_edicion: part.id_edicion,
      id_pais: part.id_pais,
  ...
  const manejarSubmit = async (e) => {
    ...
    try {
      ...
      let partId = editId;
      const payload = {
        id_edicion: formData.id_edicion,
      ...
      setEditId(null);
      setFormData({ ...formData, nombre_cancion: '', nombres_artistas: '', clasifico: false, puesto_oficial: '', puntos_oficiales: '' });
      fetchParticipaciones();
    } catch (error) {
  ...
  const participacionesActuales = participaciones.slice((paginaActual - 1) * itemsPorPagina, paginaActual * itemsPorPagina);
  ...
  return (
    <main className="container mt-5 mb-5 font-sans text-white">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2 className="display-6 fw-bold">Participaciones 🎤</h2>
        <div>
          <button onClick={() => {setEditId(null); setModalAbierto(true)}} className="btn btn-primary fw-bold me-2 text-white shadow-sm">Nueva ➕</button>
      ...
      <div className="card bg-dark p-3 mb-4 border-secondary shadow">
        <div className="row g-3 align-items-end">
          <div className="col-md-3">
            <label className="form-label small fw-bold text-white">Edición</label>
            <select className="form-select bg-dark text-white border-secondary" value={filtros.id_edicion} onChange={(e) => setFiltros({...filtros, id_edicion: e.target.value})}>
              <option value="Todos">Todas</option>
              {ediciones.map(ed => <option key={ed.id_edicion} value={ed.id_edicion}>{ed.anio} - {ed.tipo}</option>)}
            </select>
          </div>
          <div className="col-md-6">
            <label className="form-label small fw-bold text-white">Buscar Canción / Artista / País</label>
            <input 
              type="text" 
              className="form-control bg-dark text-white border-secondary" 
              value={filtros.busqueda} 
              onChange={(e) => setFiltros({...filtros, busqueda: e.target.value})} 
              placeholder="🔍 Buscar por país, canción o artista..." 
            />
          </div>
          <div className="col-md-3 d-flex gap-2">
            <button onClick={() => fetchParticipaciones()} className="btn btn-primary fw-bold flex-grow-1 text-white shadow-sm">Filtrar</button>
          ...
          <tbody>
            {participacionesActuales.map((part) => (
              <tr key={part.id_participacion}>
                ...
                <td>
                  <div className="fw-bold">{part.canciones?.nombre}</div>
          ...
          <ul className="pagination pagination-sm m-0 shadow-sm">
            ...
            {[...Array(totalPaginas)].map((_, i) => (
              <li key={i} className={`page-item ${paginaActual === i + 1 ? 'active' : ''}`}>
                <button className={`page-link ${paginaActual === i + 1 ? 'bg-primary border-primary' : 'bg-dark text-white border-secondary'}`} onClick={() => setPaginaActual(i + 1)}>{i + 1}</button>
              </li>
            ))}
      ...
      {modalAbierto && (
        <div className="position-fixed top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center" style={{ backgroundColor: 'rgba(0,0,0,0.85)', zIndex: 1050 }}>
          <div className="card bg-dark border-light shadow-lg w-100 m-3 shadow" style={{ maxWidth: '600px' }}>
            ...
            <div className="card-body p-4">
              <form onSubmit={manejarSubmit}>
                <div className="row g-3 mb-3">
                  ...
                  <div className="col-md-6">
                    <label className="form-label small fw-bold text-white">PAÍS</label>
                ...
                <div className="mb-3 position-relative">
                  <label className="form-label small fw-bold text-white">CANCIÓN <span className="text-warning text-lowercase fw-normal">(se crea si no existe)</span></label>
                  <input type="text" required className="form-control bg-dark text-white border-secondary shadow-sm" value={formData.nombre_canc_ion || formData.nombre_cancion} onChange={(e) => manejarCambioCancion(e.target.value)} onBlur={() => setTimeout(() => setSugerenciasCancion([]), 200)} />
                  {sugerenciasCancion.length > 0 && (
                    <ul className="list-group position-absolute w-100 mt-1 shadow" style={{ zIndex: 1100 }}>
                      {sugerenciasCancion.map(c => <li key={c.nombre} className="list-group-item list-group-item-action bg-secondary text-white border-dark shadow-sm" onClick={() => setFormData({...formData, nombre_cancion: c.nombre})}>{c.nombre}</li>)}
                    </ul>
                  )}
                </div>
                <div className="mb-3 position-relative">
                  <label className="form-label small fw-bold text-white">ARTISTA(S) <span className="text-warning text-lowercase fw-normal">(se crean si no existen)</span></label>
                  <input type="text" required className="form-control bg-dark text-white border-secondary shadow-sm" value={formData.nombres_artistas} onChange={(e) => manejarCambioArtista(e.target.value)} onBlur={() => setTimeout(() => setSugerenciasArtista([]), 200)} />
                  {sugerenciasArtista.length > 0 && (
                    <ul className="list-group position-absolute w-100 mt-1 shadow" style={{ zIndex: 1100 }}>
                      {sugerenciasArtista.map(a => <li key={a.nombre} className="list-group-item list-group-item-action bg-secondary text-white border-dark shadow-sm" onClick={() => {
                ...
                <div className="row g-3 mb-4">
                  <div className="col-md-4 d-flex align-items-center">
                    <div className="form-check form-switch">
                      <input className="form-check-input shadow-sm" type="checkbox" id="clasificoCheck" checked={formData.clasifico} onChange={(e) => setFormData({...formData, clasifico: e.target.checked})} />
                      <label className="form-check-label fw-bold text-white" htmlFor="clasificoCheck">Clasificó</label>
                    </div>
                  </div>
                  <div className="col-md-4">
                    <label className="form-label small fw-bold text-info">PUESTO OFICIAL</label>
                    <input type="number" className="form-control bg-dark text-white border-secondary shadow-sm" value={formData.puesto_oficial} onChange={(e) => setFormData({...formData, puesto_oficial: e.target.value})} placeholder="Ej: 5" />
                  </div>
                  <div className="col-md-4">
                    <label className="form-label small fw-bold text-info">PUNTOS</label>
                    <input type="number" className="form-control bg-dark text-white border-secondary shadow-sm" value={formData.puntos_oficiales} onChange={(e) => setFormData({...formData, puntos_oficiales: e.target.value})} placeholder="Ej: 240" />
                  </div>
                </div>

                <div className="d-flex justify-content-end gap-2 pt-2">
                  <button type="button" onClick={() => setModalAbierto(false)} className="btn btn-outline-light fw-bold shadow-sm">Cancelar</button>
                  <button type="submit" className="btn btn-warning fw-bold text-dark shadow-sm" disabled={cargando}>
                    {cargando ? 'Guardando...' : editId ? 'Guardar Cambios 💾' : 'Crear Participación 💾'}
                  </button>
                </div>
              </form>
            </div>
